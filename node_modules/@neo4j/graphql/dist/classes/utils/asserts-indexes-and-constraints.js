...
exports.parseLegacyConstraint = void 0;
const debug_1 = __importDefault(require("debug"));
const constants_1 = require("../../constants");
const debug = debug_1.default(constants_1.DEBUG_EXECUTE);
function parseLegacyConstraint(record) {
    ...
    const constraint = detailsKeys.reduce((c, kv) => {
        ...
        switch (k) {
            case "id":
                return Object.assign(Object.assign({}, c), { [k]: parseInt(v, 10) });
            case "name":
            case "type": {
                ...
                if (value === "UNIQUENESS") {
                    return Object.assign(Object.assign({}, c), { [k]: value, entityType: "NODE" });
                }
                return Object.assign(Object.assign({}, c), { [k]: value });
            }
            case "schema": {
                // Parse the format of schema string, example given above
                const match = /^\(:(?<label>.+) {(?<property>.+)}\)$/gm.exec(v);
                const label = (_a = match === null || match === void 0 ? void 0 : match.groups) === null || _a === void 0 ? void 0 : _a.label;
                const property = (_b = match === null || match === void 0 ? void 0 : match.groups) === null || _b === void 0 ? void 0 : _b.property;
                return Object.assign(Object.assign({}, c), { labelsOrTypes: [label], properties: [property] });
            }
            case "ownedIndex":
                return Object.assign(Object.assign({}, c), { ownedIndexId: parseInt(v, 10) });
            default:
...
exports.parseLegacyConstraint = parseLegacyConstraint;
function createConstraints({ nodes, session }) {
    return __awaiter(this, void 0, void 0, function* () {
            ...
            if (node.fulltextDirective) {
                node.fulltextDirective.indexes.forEach((index) => {
                    const properties = index.fields.map((field) => {
                        ...
                        return (stringField === null || stringField === void 0 ? void 0 : stringField.dbPropertyName) || field;
                    });
        ...
        for (const indexToCreate of indexesToCreate) {
            ...
            debug(`About to execute Cypher: ${cypher}`);
            // eslint-disable-next-line no-await-in-loop
            yield session.run(cypher);
            debug(`Created @fulltext index ${indexToCreate.indexName}`);
        }
    });
}
function checkConstraints({ nodes, session }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const constraintsResult = yield session.run(constrainsCypher);
        ...
        nodes.forEach((node) => {
            node.uniqueFields.forEach((field) => {
                ...
                if (!((_a = existingConstraints[node.getMainLabel()]) === null || _a === void 0 ? void 0 : _a.includes(property))) {
        ...
        const indexesResult = yield session.run(indexesCypher);
            ...
            if (node.fulltextDirective) {
                node.fulltextDirective.indexes.forEach((index) => {
                    ...
                    index.fields.forEach((field) => {
                        const stringField = node.primitiveFields.find((f) => f.fieldName === field);
                        const fieldName = (stringField === null || stringField === void 0 ? void 0 : stringField.dbPropertyName) || field;
                        const property = existingIndex.properties.find((p) => p === fieldName);
                        if (!property) {
                            const aliasError = (stringField === null || stringField === void 0 ? void 0 : stringField.dbPropertyName) ? ` aliased to field '${fieldName}''` : "";
...
function assertIndexesAndConstraints({ driver, driverConfig, nodes, options, }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if (options === null || options === void 0 ? void 0 : options.create) {
                yield createConstraints({ nodes, session });
            }
            else {
                yield checkConstraints({ nodes, session });
            }
        }
        finally {
            yield session.close();
