    ...
    relationshipFields.forEach((rel) => {
        ...
        if (rel.interface) {
            ...
            const connectFieldInput = schemaComposer.getOrCreateITC(`${sourceName}${graphql_compose_1.upperFirst(rel.fieldName)}ConnectFieldInput`, (tc) => {
                tc.addFields(Object.assign(Object.assign(Object.assign({}, (schemaComposer.has(`${rel.typeMeta.name}ConnectInput`)
            ...
            const deleteFieldInput = schemaComposer.getOrCreateITC(`${sourceName}${graphql_compose_1.upperFirst(rel.fieldName)}DeleteFieldInput`, (tc) => {
                tc.addFields(Object.assign(Object.assign({}, (schemaComposer.has(`${rel.typeMeta.name}DeleteInput`)
            ...
            const disconnectFieldInput = schemaComposer.getOrCreateITC(`${sourceName}${graphql_compose_1.upperFirst(rel.fieldName)}DisconnectFieldInput`, (tc) => {
                tc.addFields(Object.assign(Object.assign({}, (schemaComposer.has(`${rel.typeMeta.name}DisconnectInput`)
            ...
            schemaComposer.getOrCreateITC(`${sourceName}${graphql_compose_1.upperFirst(rel.fieldName)}UpdateConnectionInput`, (tc) => {
                tc.addFields(Object.assign(Object.assign({}, (rel.properties ? { edge: `${rel.properties}UpdateInput` } : {})), { node: `${rel.typeMeta.name}UpdateInput` }));
            });
            ...
            refNodes.forEach((n) => {
                ...
                if (!schemaComposer.has(createName)) {
                    schemaComposer.createInputTC({
                        name: createName,
                        fields: Object.assign({ node: `${n.name}CreateInput!` }, (rel.properties ? { edge: `${rel.properties}CreateInput!` } : {})),
                    });
                }
            });
            ...
            fields.forEach((field) => {
                ...
                if (constants_1.WHERE_AGGREGATION_AVERAGE_TYPES.includes(field.typeMeta.name)) {
                    aggregationInput.addFields(constants_1.WHERE_AGGREGATION_OPERATORS.reduce((res, operator) => {
                        ...
                        return Object.assign(Object.assign(Object.assign({}, res), { [`${field.fieldName}_${operator}`]: field.typeMeta.name, [`${field.fieldName}_AVERAGE_${operator}`]: averageType, [`${field.fieldName}_MIN_${operator}`]: field.typeMeta.name, [`${field.fieldName}_MAX_${operator}`]: field.typeMeta.name }), (field.typeMeta.name !== "Duration"
                            ? { [`${field.fieldName}_SUM_${operator}`]: field.typeMeta.name }
                            : {}));
                    }, {}));
                    return;
                }
                aggregationInput.addFields(constants_1.WHERE_AGGREGATION_OPERATORS.reduce((res, operator) => (Object.assign(Object.assign({}, res), { [`${field.fieldName}_${operator}`]: field.typeMeta.name, [`${field.fieldName}_MIN_${operator}`]: field.typeMeta.name, [`${field.fieldName}_MAX_${operator}`]: field.typeMeta.name })), {}));
            });
        ...
        whereInput.addFields(Object.assign({
        ...
        schemaComposer.getOrCreateITC(createName, (tc) => {
            tc.addFields(Object.assign({ node: `${n.name}CreateInput!` }, (hasNonGeneratedProperties
                ? { edge: `${rel.properties}CreateInput${hasNonNullNonGeneratedProperties ? `!` : ""}` }
                : {})));
        });
        const connectWhereName = `${n.name}ConnectWhere`;
        ...
        schemaComposer.getOrCreateITC(connectName, (tc) => {
            tc.addFields(Object.assign(Object.assign({ where: connectWhereName }, (n.relationFields.length
        ...
        schemaComposer.getOrCreateITC(connectionUpdateInputName, (tc) => {
            tc.addFields(Object.assign({ node: updateField }, (hasNonGeneratedProperties ? { edge: `${rel.properties}UpdateInput` } : {})));
        });
        const connectOrCreate = create_connect_or_create_field_1.createConnectOrCreateField({
            relationField: rel,
            node: n,
        ...
        if (!schemaComposer.has(nodeFieldDeleteInputName)) {
            schemaComposer.createInputTC({
                ...
                fields: Object.assign({ where: `${rel.connectionPrefix}${graphql_compose_1.upperFirst(rel.fieldName)}ConnectionWhere` }, (n.relationFields.length ? { delete: `${n.name}DeleteInput` } : {})),
            });
        }
        if (!schemaComposer.has(nodeFieldDisconnectInputName)) {
            schemaComposer.createInputTC({
                name: nodeFieldDisconnectInputName,
                fields: Object.assign({ where: `${rel.connectionPrefix}${graphql_compose_1.upperFirst(rel.fieldName)}ConnectionWhere` }, (n.relationFields.length ? { disconnect: `${n.name}DisconnectInput` } : {})),
            });
        }
        nodeRelationInput.addFields({
