...
function connectionFieldResolver({ connectionField, source, args, info, }) {
    ...
    return Object.assign({ [totalCountKey]: utils_1.isNeoInt(totalCount) ? totalCount.toNumber() : totalCount }, createConnectionWithEdgeProperties({ source: value, selectionSet, args, totalCount }));
}
...
function createOffsetLimitStr({ offset, limit, }) {
    ...
    if (hasLimit && hasOffset) {
        const sliceStart = utils_1.isNeoInt(offset) ? offset.toNumber() : offset;
        const itemsToGrab = utils_1.isNeoInt(limit) ? limit.toNumber() : limit;
