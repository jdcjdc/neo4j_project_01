...
function createFieldAggregation({ context, nodeLabel, node, field, }) {
    ...
    if (!relationAggregationField || !connectionField)
        return undefined;
    const referenceNode = utils_1.getReferenceNode(context, relationAggregationField);
    const referenceRelation = utils_1.getReferenceRelation(context, connectionField);
    if (!referenceNode || !referenceRelation)
        return undefined;
    const fieldPathBase = `${node.name}${referenceNode.name}${graphql_compose_1.upperFirst(relationAggregationField.fieldName)}`;
    const aggregationFields = getAggregationFields(fieldPathBase, field);
    const authData = field_aggregations_auth_1.createFieldAggregationAuth({
        node: referenceNode,
        context,
        subqueryNodeAlias,
        nodeFields: aggregationFields.node,
    });
    const [whereQuery, whereParams] = create_where_and_params_1.default({
    ...
    const matchWherePattern = aggregation_sub_queries_1.createMatchWherePattern(targetPattern, authData, whereQuery);
    const apocRunParams = Object.assign(Object.assign({}, apoc_run_1.serializeParamsForApocRun(whereParams)), utils_1.serializeAuthParamsForApocRun(authData));
    return {
        query: stringify_object_1.stringifyObject({
        ...
        params: Object.assign(Object.assign({}, authData.params), whereParams),
    };
}
...
function getAggregationFields(fieldPathBase, field) {
    ...
    const node = (_a = utils_1.getFieldByName("node", aggregationFields)) === null || _a === void 0 ? void 0 : _a.fieldsByTypeName[`${fieldPathBase}${field_aggregation_composer_1.FieldAggregationSchemaTypes.node}`];
    const edge = (_b = utils_1.getFieldByName("edge", aggregationFields)) === null || _b === void 0 ? void 0 : _b.fieldsByTypeName[`${fieldPathBase}${field_aggregation_composer_1.FieldAggregationSchemaTypes.edge}`];
    const count = utils_1.getFieldByName("count", aggregationFields);
    return { count, edge, node };
}
function createTargetPattern({ nodeLabel, relationField, referenceNode, context, }) {
...
function createCountQuery({ nodeLabel, matchWherePattern, targetAlias, params, }) {
    const apocCount = apoc_run_1.wrapInApocRunFirstColumn(AggregationSubQueries.countQuery(matchWherePattern, targetAlias), Object.assign(Object.assign({}, params), { [nodeLabel]: nodeLabel }));
...
function createAggregationQuery({ nodeLabel, matchWherePattern, fields, fieldAlias, graphElement, params, }) {
    const fieldsSubQueries = Object.values(fields).reduce((acc, field) => {
        const fieldType = utils_1.getFieldType(field);
        const dbProperty = map_to_db_property_1.default(graphElement, field.name);
        const aggregationQuery = apoc_run_1.wrapInApocRunFirstColumn(getAggregationSubquery({
            matchWherePattern,
            fieldName: dbProperty || field.name,
            type: fieldType,
            targetAlias: fieldAlias,
        }), Object.assign(Object.assign({}, params), { [nodeLabel]: nodeLabel }));
        acc[field.alias] = `head(${aggregationQuery})`;
        return acc;
    }, {});
    return stringify_object_1.stringifyObject(fieldsSubQueries);
}
