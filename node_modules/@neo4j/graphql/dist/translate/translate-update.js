...
function translateUpdate({ node, context }) {
    ...
    let projStr = "";
    let cypherParams = {};
    const topLevelMatch = translate_top_level_match_1.default({ node, context, varName, operation: "UPDATE" });
    matchAndWhereStr = topLevelMatch[0];
    cypherParams = Object.assign(Object.assign({}, cypherParams), topLevelMatch[1]);
    const connectionStrs = [];
    const interfaceStrs = [];
    let updateArgs = {};
    const mutationResponse = resolveTree.fieldsByTypeName[`Update${node.getPlural({ camelCase: false })}MutationResponse`];
    const nodeProjection = Object.values(mutationResponse).find((field) => field.name === node.getPlural({ camelCase: true }));
    if (updateInput) {
        ...
        Object.entries(disconnectInput).forEach((entry) => {
            ...
            if (relationField.union) {
                ...
                (_b = (_a = relationField.interface) === null || _a === void 0 ? void 0 : _a.implementations) === null || _b === void 0 ? void 0 : _b.forEach((implementationName) => {
                    refNodes.push(context.neoSchema.nodes.find((x) => x.name === implementationName));
                });
            }
            else {
    ...
    if (connectOrCreateInput) {
        Object.entries(connectOrCreateInput).forEach(([key, input]) => {
            ...
            refNodes.forEach((refNode) => {
                const connectAndParams = create_connect_or_create_and_params_1.createConnectOrCreateAndParams({
    ...
    if (nodeProjection === null || nodeProjection === void 0 ? void 0 : nodeProjection.fieldsByTypeName) {
        ...
        if ((_f = (_e = projection[2]) === null || _e === void 0 ? void 0 : _e.interfaceFields) === null || _f === void 0 ? void 0 : _f.length) {
            projection[2].interfaceFields.forEach((interfaceResolveTree) => {
                ...
                const interfaceProjection = create_interface_projection_and_params_1.default({
                ...
                cypherParams = Object.assign(Object.assign({}, cypherParams), interfaceProjection.params);
            });
        }
    }
